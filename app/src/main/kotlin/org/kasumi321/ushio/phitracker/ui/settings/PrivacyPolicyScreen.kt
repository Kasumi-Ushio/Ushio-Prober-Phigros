...
fun PrivacyPolicyScreen(
    ...
    Scaffold(
        ...
        Column(
            ...
            PolicyText("最后更新日期：2026 年 3 月 14 日")
            ...
            PolicyText("1. 读取并展示您的 Phigros 游戏存档与成绩分析（B30、RKS 计算等）。")
            PolicyText("2. 存储您的个性化设置，以便在下次启动时恢复。")
            PolicyText("3. 缓存曲绘资源，减少网络流量消耗。")

            PolicyTitle("三、数据的存储与安全")
            PolicyText("所有数据均存储在您的设备本地。本应用不会将您的数据上传至任何我们所控制的远程服务器。")
            PolicyText("具体而言：")
            PolicyText("1. 游戏存档和成绩数据存储在应用本地的 Room 数据库中。")
            PolicyText("2. 设置偏好存储在 Android SharedPreferences 中。")
            PolicyText("3. 曲绘缓存存储在应用的专用缓存目录中。")
            PolicyText("4. sessionToken 存储在应用的本地数据库中，且不会以任何形式被传输到第三方。")
            ...
            PolicyText("3. GitHub（github.com）：用于获取曲绘图片资源和曲目数据更新。")
            PolicyText("4. gh-proxy（gh-proxy.com）：用于在中国内地环境中加速请求和获取 GitHub 资源。"  )
            PolicyText("本应用不使用任何第三方分析、广告或追踪 SDK。")

            PolicyTitle("五、您的权利")
            PolicyText("您可以随时：")
            PolicyText("1. 退出登录，清除本地存储的账号凭证。")
            PolicyText("2. 在设置中清理本地缓存的曲绘资源。")
            PolicyText("3. 通过 Android 系统的\"应用信息\"功能清除本应用的全部本地数据。")
            PolicyText("4. 卸载本应用以完全删除所有相关数据。")
